import { app, dialog, globalShortcut, nativeImage, Tray } from 'electron';
...
export class MainWindow extends BaseWindow {
  ...
  private registerShortCuts() {
    globalShortcut.register('Ctrl+Alt+F5', () => this.toggleVisibility());
    globalShortcut.register('Ctrl+F5', () => this.window.reload());
  }
  ...
  constructor() {
    ...
    this.window.on('unmaximize', () => this.window.maximize());
  ...
  async show() {
    // await this.window.loadFile(path.resolve(__dirname, '../../frontend/index.html'));
    await this.window.loadURL('http://localhost:8888');
